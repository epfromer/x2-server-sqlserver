...
export async function getAllEmail(req: Request, res: Response): Promise<void> {
  try {
    ...
    const params = { TableName: dbName + emailCollection }
    ...
    result.Items.forEach((email) => {
      emails.push({
        id: email.id.S,
        sent: new Date(+email.sent.N).toISOString(),
        sentShort: new Date(+email.sent.N).toISOString().slice(0, 10),
        from: email.from.S,
        fromCustodian: email.fromCustodian ? email.fromCustodian.S : '',
        to: email.to.S,
        toCustodians: email.toCustodians.S
          ? email.toCustodians.S.split(',')
          : [],
        cc: email.cc.S,
        bcc: email.bcc.S,
